...
if($clave!=$reclave){
	...
	{
		echo "<script language=JavaScript>alert('El nombre usuario ya esta en uso\\nPor favor elija otro');window.location='formulariodepartamento.php';</script>";	
	} 
	if(mysql_num_rows($resultado)==0){	
		...
		if(mysql_num_rows($resultado1)>0){		
			echo "<script language=JavaScript>alert('La cedula ya esta registrada en el sistema');window.location='formulariodepartamento.php';</script>";
		}
		if(mysql_num_rows($resultado1)==0){		
			//tabla usuario
			$fecha = date("Y-n-j H:i:s");
			$sql = "INSERT INTO usuario(id_grupo,login,password,fecha_registro)"."VALUES (2,'$nombreusuario','$clave','$fecha')";
			$result = mysql_query($sql);		
			$ultimo_id = mysql_insert_id();

			$modu_aud='REGISTRO DEPARTAMENTO';
            $tabl_aud='usuario';
            $acci_aud='CREAR';
            $fech_aud=date('Y/m/d');
            $hora_aud=date('G:i:s');
			...
			//tabla departamento
			$sql = "INSERT INTO departamento(cedula_departamento,id_usuario,nombre,apellido,cargo,email)"."VALUES ('$cedula','$ultimo_id','$nombre','$apellido','$cargo','$email')";
			$result = mysql_query($sql);		
			
			$modu_aud='REGISTRO DEPARTAMENTO';
            $tabl_aud='departamento';
            $acci_aud='CREAR';
            $fech_aud=date('Y/m/d');
            $hora_aud=date('G:i:s');
            $usua_aud=$_SESSION['id'];
