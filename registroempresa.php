...
if($clave!=$reclave){
	...
	{
		echo "<script language=JavaScript>alert('El nombre usuario ya esta en uso\\nPor favor elija otro');window.location='formularioempresa.php';</script>";	
	} 
	if(mysql_num_rows($resultado)==0){	
		$consulta="select * from empresa where rif='$rif'";
		$resultado1=mysql_query($consulta) or die (mysql_error());
		if(mysql_num_rows($resultado1)>0){		
			echo "<script language=JavaScript>alert('El rif ya esta registrado en el sistema');window.location='formularioempresa.php';</script>";
		}
		if(mysql_num_rows($resultado1)==0){		
			//tabla usuario
			$fecha = date("Y-n-j H:i:s");
			$sql = "INSERT INTO usuario(id_grupo,login,password,fecha_registro)"."VALUES (3,'$nombreusuario','$clave','$fecha')";
			$result = mysql_query($sql);		
			$ultimo_id = mysql_insert_id();		

			$modu_aud='REGISTRO EMPRESA';
            $tabl_aud='usuario';
            $acci_aud='CREAR';
            $fech_aud=date('Y/m/d');
            $hora_aud=date('G:i:s');
			...
			//tabla departamento
			$sql = "INSERT INTO empresa(rif,id_usuario,nombre_empresa,direccion_empresa,telefono_empresa,email_empresa)"."VALUES ('$rif','$ultimo_id','$nombre','$direccion','$telefono','$email')";
			$result = mysql_query($sql);
